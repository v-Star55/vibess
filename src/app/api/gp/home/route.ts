...
function calculatePriorityScore(gp: any, distance: number): number {
  let score = 0;
  const now = new Date();
...
export async function GET(req: NextRequest) {
  try {
    ...
    const gpsWithScores = foundGPs.map((gp) => {
      ...
      const priorityScore = calculatePriorityScore(gp, distance);
    ...
    // Group by category and get top 3-4 per category
    const gpsByCategory: Record<string, any[]> = {
      "Vibe GP": [],
      "Movie GP": [],
      "Anime GP": [],
      "Other GP": [],
    ...
    // Format response with only categories that have GPs
    const formattedGPs: Record<string, any[]> = {};
    Object.keys(gpsByCategory).forEach((category) => {
      if (gpsByCategory[category].length > 0) {
        formattedGPs[category] = gpsByCategory[category];
      }
    });
    ...
    console.error("GP Home Error:", error);
    return NextResponse.json(
